...
const { assert, bigNumberify, debug, eq, ge, lt, getViewsHelper, deferContract, makeRandom, argsSplit } = shared;
...
export function makeEthLike(ethLikeArgs) {
  ...
  const transfer = async (from, to, value, token = false) => {
  ...
  const connectAccount = async (networkAccount) => {
    // @ts-ignore // TODO
      ...
      const recv_impl = async (okNum, out_tys, waitIfNotPresent, timeout_delay) => {
        ...
        while (!timeout_delay || lt(block_poll_start, add(lastBlock, timeout_delay))) {
          ...
          if (es.length == 0) {
            ...
            const data = T_Tuple(out_tys).unmunge(ok_vals); // TODO: typing
  ...
  const newAccountFromSecret = async (secret) => {
    const provider = await getProvider();
    const networkAccount = (new ethers.Wallet(secret)).connect(provider);
    const acc = await connectAccount(networkAccount);
    return acc;
  };
  const newAccountFromMnemonic = async (phrase) => {
    const provider = await getProvider();
  ...
  const fundFromFaucet = async (account, value) => {
    const faucet = await getFaucet();
    await transfer(faucet, account, value);
  };
  const newTestAccount = async (startingBalance) => {
  ...
  // onProgress callback is optional, it will be given an obj
  // {currentTime, targetTime}
  const wait = async (delta, onProgress) => {
    const now = await getNetworkTime();
    return await waitUntilTime(add(now, delta), onProgress);
  };
  // onProgress callback is optional, it will be given an obj
  // {currentTime, targetTime}
  const waitUntilTime = async (targetTime, onProgress) => {
  ...
  const verifyContract = async (ctcInfo, backend) => {
  ...
  const minimumBalance = parseCurrency(0);
  /**
   ...
   */
  function formatCurrency(amt, decimals = standardDigits) {
