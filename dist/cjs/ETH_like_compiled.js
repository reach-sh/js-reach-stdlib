...
function makeEthLikeCompiled(ethLikeCompiledArgs) {
    ...
    var V_Null = null;
    var T_Null = __assign(__assign({}, CBR.BT_Null), { defaultValue: V_Null, 
        // null is represented in solidity as false
        munge: function (bv) { return (void (bv), false); }, unmunge: function (nv) { return (void (nv), V_Null); }, paramType: 'bool' });
    ...
    var T_UInt = __assign(__assign({}, CBR.BT_UInt), { defaultValue: ethers_1.ethers.BigNumber.from(0), munge: function (bv) { return bv; }, unmunge: function (nv) { return V_UInt(nv); }, paramType: 'uint256' });
    var V_UInt = function (n) {
        return T_UInt.canonicalize(n);
    };
    var T_Bytes = function (len) {
        var me = __assign(__assign({}, CBR.BT_Bytes(len)), { defaultValue: ''.padEnd(len, '\0'), munge: function (bv) { return Array.from(ethers_1.ethers.utils.toUtf8Bytes(bv)); }, unmunge: function (nv) { return me.canonicalize(shared_impl_1.hexToString(ethers_1.ethers.utils.hexlify(nv))); }, paramType: "uint8[" + len + "]" });
        return me;
    };
